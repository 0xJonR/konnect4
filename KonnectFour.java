...
public class KonnectFour{
  ...
  public KonnectFour(){
    ...
    System.out.format("Welcome To Connect 4%n");
    Random rad = new Random();
    isPlayer = rad.nextBoolean();
    if (isPlayer){
      System.out.format("Random odds have decided you go first.%n");
    }
    else{
      System.out.format("Random odds have decided you go second. %n");
    }
    while (isWon!=true){
    if (isPlayer){
      b_toString();
  ...
  public void move(int c, boolean isPlayer){
    ...
    if (isPlayer){
      ...
      if (winCheck(r,c)){
        b_toString();
        System.out.format("You Won!%n" );
        isWon=true;
    }
    }
    else {
      Board[r][c] = "[O]";
      System.out.format("Computer moves the chip to slot %d", c);
      //b_toString();
      if (winCheck(r,c)){
        b_toString();
